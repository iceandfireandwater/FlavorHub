...
class VectorSearcher:
    ...
    def search_similar(
        ...
        if metric == "l2":
            ...
            sim_expr = "1.0 / (1.0 + (embedding <-> q.v))"  # 距离→相似度转换
            dist_expr = "embedding <-> q.v"
        # 余弦距离
        else:
            order_expr = "embedding <=> q.v"
            sim_expr = "1.0 - (embedding <=> q.v)"  # 距离→相似度转换
            dist_expr = "embedding <-> q.v"

        sql = f"""
        WITH q AS (SELECT %s::vector AS v)
        SELECT
          id, source_table, source_id, metadata, content,  # 5 个字段——id/来源表/来源行 id/元数据 JSON/正文内容
        ...
        try:
            ...
            with conn.cursor() as cur:
                try:
                    cur.execute("SET ivfflat.probes = 10;")  # IVFFlat 索引探针数（召回率 vs 速度）
                except Exception:  # pragma: no cover - optional extension
                    pass
                self.logger.debug(cur.mogrify(sql, params).decode())
                cur.execute(sql, params)
                rows = cur.fetchall()  # 得到了这个表：id, source_table, source_id, metadata, content, similarity, distance
        finally:
            conn.close()

        # id, source_table, source_id, metadata, content, similarity, distance
        results = []
        for row in rows:
            # 解析 metadata
            metadata_dict = {}
    ...
    def _apply_reranker(self, query: str, results: List[dict]) -> List[dict]:
        ...
        # Reranker 不处理所有检索结果，只处理前 max_candidates 条
        ...
        if fusion_alpha is not None and 0 < fusion_alpha < 1:
            ...
            # Z-score 标准化
            # 因为 embedding score 和 rerank score 不在一个量级上，所以需要分别标准化
