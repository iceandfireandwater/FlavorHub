...
class CRUDChatMessage(CRUDBase[ChatMessage, ChatMessageCreate, ChatMessageUpdate]):
    ...
    # 获取同一个对话框里的最新一条消息，根据 session_id 和 order_index 获得
    def get_latest_by_session(
        ...
        """
        return (
            db.query(self.model)                          # SELECT ... FROM chat_messages
            .filter(self.model.session_id == session_id)  # WHERE session_id = ?
            .order_by(desc(self.model.order_index))       # ORDER BY order_index DESC
            .first()                                      # LIMIT 1
        )
