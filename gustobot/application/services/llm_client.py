...
# 它把 OpenAI API 的调用细节封装起来，让项目其他地方不用关心 API Key、Base URL、消息格式、JSON 输出解析等问题，只需要调用 chat() 或 chat_json()
        #         LangGraph Agent
    ...
    async def chat_json(
        ...
        *,
        ...
        try:
            ...
            response = await self.complete(messages=messages, temperature=temperature)  # 失败，自动降级为纯文本并重试
            content = response.choices[0].message.content or ""
