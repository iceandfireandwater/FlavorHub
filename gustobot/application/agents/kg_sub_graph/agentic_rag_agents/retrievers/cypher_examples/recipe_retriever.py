...
class RecipeCypherRetriever(BaseCypherExampleRetriever):
    ...
    def get_examples(self, query: str, k: int = 5) -> str:
        ...
        question_type = classification.question_type  # 有 recipe_property, property_constraint, relationship_constraint, relationship_query
...
ORDER BY 使用次数 DESC LIMIT 5"""
        ...
        try:
            ...
            generated_cyphers = parsed.get("sql", [])  # 这里就形成了一个完整的 cypher 语句

            if generated_cyphers:
                # 将生成的 Cypher 作为第一个示例，因为质量最好
