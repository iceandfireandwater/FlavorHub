...
def create_guardrails_node(
    ...
    # Callable[[InputState], ...]: 这是一个函数，它接收一个名为 InputState 的参数。
    # Coroutine[Any, Any, dict[str, Any]]:
    # 这个函数是一个异步函数（async def）。
    # Coroutine 的前两个 Any 通常代表内部暂停（yield）和发送（send）的类型（在大多数应用层代码中不重要，填 Any 即可）。
    # 最后一个参数 dict[str, Any]：这才是关键！它表示这个协程执行完毕（await 结束）后返回的具体数据类型。
    """
    ...
    # 参考此范围描述来决策：scope_description；参考图表结构来回答：graph
