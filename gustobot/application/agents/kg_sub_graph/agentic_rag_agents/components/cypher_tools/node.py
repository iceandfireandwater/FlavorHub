...
# 定义GraphRAG API包装器
# LangGraph 后面会 graph.add_node("cypher_query",create_cypher_query_node())
def create_cypher_query_node(
    ...
    async def cypher_query(
        ...
        # 连接信息一律从环境变量读（不要硬编码凭据）
        import os as _os

        NEO4J_URI = _os.getenv("NEO4J_URI", "bolt://localhost:7687")
        NEO4J_USERNAME = _os.getenv("NEO4J_USER", "neo4j")
        NEO4J_PASSWORD = _os.getenv("NEO4J_PASSWORD", "")
        NEO4J_DATABASE = _os.getenv("NEO4J_DATABASE", "neo4j")
