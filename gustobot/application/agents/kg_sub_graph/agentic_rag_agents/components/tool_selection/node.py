...
def create_tool_selection_node(
    ...
    tool_selection_chain: Runnable[Dict[str, Any], Any] = (
        tool_selection_prompt # 写了4种工具的简介
    ...
    async def tool_selection(
        ...
        # 注意：这里原本有个 heuristic，把「菜谱描述类」问题（命中 食材/材料/营养 等词）
        # 直接强制路由到 microsoft_graphrag_query（LightRAG）。但底层 LightRAG 对象
        # 缺少 `_addon_params`（版本不兼容），调用必抛
        # "执行 LightRAG 查询时出错: 'LightRAG' object has no attribute '_addon_params'"，
        # 于是同一个问题会**时好时坏**：LLM 改写后的查询串里恰好带上「食材」就挂，
        # 不带就走下面的工具选择（实测会选 predefined_cypher 查 Neo4j，能正常答出步骤）。
        # 例：「土豆烧茄子怎么做」→ 改写为「土豆烧茄子的做法步骤、所需食材及用量是什么」
        #     → 命中「食材」→ 走 LightRAG → 失败 → 回答"没查到"。
        # 因此不再强推 LightRAG，交给 LLM 工具选择 / 后续 fallback 决定。
        # （若以后修好 LightRAG，可恢复此分支，但要先加"可用性检测"。）
        ...
        if tool_selection_output is not None:
            ...
            logger.info(
                "LLM 选择工具",  # cypher_query， predefined_cypher， text2sql_query， customer_tools(自定义工具)
                extra={
                    "tool_name": tool_name,
                    "tool_args": tool_args,
                    "route_type": route_type or "unknown",
            ...
            logger.info("LLM 选择了工具：{}", tool_name)
