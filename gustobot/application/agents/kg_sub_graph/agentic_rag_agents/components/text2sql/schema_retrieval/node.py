...
async def _retrieve_schema_from_mysql(question: str) -> Dict[str, Any]:
    ...
    def _sync_retrieve():
        ...
        try:
            with engine.connect() as conn:
                ...
                # [('chat_history_snapshots', ''), ('chat_messages', ''), ('chat_sessions', ''), ('conversation_history_snapshots', ''), ('conversations', ''), ('cooking_tools', ''), ('cuisines', ''), ('ingredients', ''), ('messages', ''), ('recipe_ingredients', ''), ('recipe_steps', ''), ('recipes', ''), ('step_tools', ''), ('users', '')]
                logger.info("获取所有表信息：{}", table_rows)
                ...
                for table_row in table_rows:
                    ...
                    # [('id', 'int', '', 'PRI'), ('session_id', 'varchar', 'Parent session ID', 'MUL'), ('message_type', 'varchar', 'Message type: user_query, agent_response, knowledge, error, etc.', 'MUL'), ('content', 'text', 'Message content text', ''), ('message_metadata', 'json', 'Additional metadata: route info, confidence, sources, etc.', ''), ('order_index', 'int', 'Message order within the session', ''), ('created_at', 'datetime', 'Message creation timestamp', '')]
                    logger.info("获取表 {} 的列信息：{}", table_name, column_rows)
                ...
                # [('chat_history_snapshots', 'session_id', 'chat_sessions', 'id'), ('chat_messages', 'session_id', 'chat_sessions', 'id'), ('conversation_history_snapshots', 'conversation_id', 'conversations', 'id'), ('conversations', 'user_id', 'users', 'id'), ('messages', 'conversation_id', 'conversations', 'id'), ('recipe_ingredients', 'ingredient_id', 'ingredients', 'id'), ('recipe_ingredients', 'recipe_id', 'recipes', 'id'), ('recipe_steps', 'recipe_id', 'recipes', 'id'), ('recipes', 'cuisine_id', 'cuisines', 'id'), ('step_tools', 'step_id', 'recipe_steps', 'id'), ('step_tools', 'tool_id', 'cooking_tools', 'id')]
                logger.info("获取外键关系：{}", fk_rows)
    ...
    logger.info("Schema 检索结果（关联性最高的几张表）：{}", schema_context)
