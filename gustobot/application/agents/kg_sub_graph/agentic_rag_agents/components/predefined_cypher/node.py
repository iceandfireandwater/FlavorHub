...
def create_predefined_cypher_node(
    ...
    async def predefined_cypher(
        ...
        question = state.get("task", "")  # task中的问题
        incoming_params = state.get("query_parameters", {}) or {}
        query_name = incoming_params.get("query") or state.get("query_name", "")

        # 获取和用户question最匹配的模板描述
        if not query_name:
            matches = matcher.match_query(question, top_k=1)  # 计算 question 和 模板描述 的cosine-similarity
            if matches:
                query_name = matches[0]["query_name"]  # 获得最匹配的query_name
            else:
        ...
        #     }
        # ]
        statement = predefined_cypher_dict.get(query_name) if query_name else None  # 根据query_name获取对应的Cypher模板
        # """
        # MATCH (d:Dish{name:$dish_name})
        # RETURN d.instructions
        # """
        parameters: Dict[str, Any] = incoming_params.get("parameters") or {}  # 获取传入的参数
