...
class VectorQueryMatcher:
    ...
    def __init__(
        self,
        predefined_cypher_dict: Dict[str, str],  # 预定义好的 cypher 模板库
        query_descriptions: Dict[str, str],  # 查询描述，模板名+自然语言描述
    ...
    # 把所有 Cypher 查询类型描述转换成向量，模板 → vector
    def _compute_query_vectors(self) -> Dict[str, np.ndarray]:
        ...
        for query_name in self.predefined_cypher_dict:
            ...
            corpus.append(f"{query_name} {description}".strip())  # 把 query_name 和 description 连起来，一起向量化
        ...
        # key:query_name, value:vectorized representation of query_name and its description
        return {
            key: np.asarray(vector, dtype=np.float32) for key, vector in zip(keys, matrix)
        }

        # {
        #     "dish_instructions": [0.2, 0.5, ...],
        #     "dish_cook_time": [0.1, 0.8, ...],
        # }   
    ...
    # user_question 和 query_name 的相似度匹配，返回相似度最高的 top_k 个 query_name
    def match_query(self, user_question: str, top_k: int = 3) -> List[Dict[str, Any]]:
        ...
        # query_name:query_name, vector:vectorized representation of query_name and its description
        ...
        # 先用top_k过滤，再用similarity_threshold过滤
