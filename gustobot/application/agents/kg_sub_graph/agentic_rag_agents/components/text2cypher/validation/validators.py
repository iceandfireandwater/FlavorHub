...
async def validate_cypher_query_with_llm(
    ...
    # 这个validate_cypher_chain；里面有专门用于检查 cypher 语法的prompt
    ...
    if llm_output.errors:
        errors.extend(llm_output.errors)  # cypher 本身的语法错误

    # 把 filters 里的每个"过滤条件"拿去数据库核对
    if llm_output.filters:
        for filter in llm_output.filters:  # 遍历每个过滤条件
            # Do mapping only for string values
            # ① 查 schema：Dish.name 是不是 STRING 类型？
            if (
                not [
                    prop
                ...
                continue  # 非字符串不查（数字没法大小写比较）

             # ② 执行探针：库里到底有没有叫"麻婆豆腐"的 Dish？
            mapping = graph.query(
            ...
            if not mapping:
                ...
                mapping_errors.append(mapping_error)  # cypher查询的东西在graph里根本没有
...
def validate_cypher_query_with_schema(
    ...
    # nodes: [ {label: "Dish",    property_name: "name",  property_value: "麻婆豆腐"},
    #      {label: "Flavor",  property_name: None,    property_value: None} ]
    # relationships: [ {type: "HAS_FLAVOR", ...} ]
