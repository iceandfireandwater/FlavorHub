...
# 和 Cypher 的本质区别：
# Cypher：MATCH (d:Dish {name:'麻婆豆腐'})-[:HAS_FLAVOR]->(f) —— 精确、要你知道结构
# LightRAG：把"麻婆豆腐"向量化 → 在图谱索引里找相关实体 → 沿边扩散找邻居 → 汇总成摘要 —— 不用写查询语句，靠语义
...
class LightRAGAPI:
    ...
    def __init__(
        ...
        retrieval_mode: Optional[str] = None,  # 检索模式：local, global, hybrid, naive, mix, bypass
        ...
        self.working_dir = working_dir or settings.LIGHTRAG_WORKING_DIR  # data\lightrag
        self.retrieval_mode = retrieval_mode or settings.LIGHTRAG_RETRIEVAL_MODE  # hybrid
        self.top_k = top_k or settings.LIGHTRAG_TOP_K  # 10
        self.max_token_size = max_token_size or settings.LIGHTRAG_MAX_TOKEN_SIZE  # 4096
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.rag = LightRAG(
                ...
                llm_model_func=self._llm_model_func,  # 定义的 LLM 模型函数
                embedding_func=EmbeddingFunc(  # 定义的 Embedding 函数
                    embedding_dim=embedding_dim,
                    max_token_size=self.max_token_size,
                    func=self._embedding_func,  # 定义的 Embedding 函数
                ),
            ...
            await self.rag.initialize_storages()   # ← 把索引文件读进内存
    ...
    async def query(self, query: str, mode: Optional[str] = None) -> Dict[str, Any]:
        ...
        # 先初始化，把 E:\code\python\project\GustoBot\data\lightrag 里的索引文件读进内存
        await self.initialize()

        retrieval_mode = mode or self.retrieval_mode  # hybrid
        ...
        try:
            ...
            # aquery(query, param)
            #     ├─ embedding 编码 query
            #     ├─ 向量检索：在图谱索引里找相关实体/chunk（vdb_chunks）
            #     ├─ 图扩散：沿实体关系找邻居（local 模式）或查社区摘要（global 模式）
            #     └─ LLM 生成：把检索到的上下文 + 问题 → 生成摘要回答（复用 _llm_model_func）
...
def create_lightrag_query_node(
    ...
    async def lightrag_query(
        ...
        logger.info(f"LightRAG_query: 查询文本: {query}")
