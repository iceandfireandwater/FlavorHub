...
async def _drop_checkpoint(session_id: str) -> bool:
    """删掉该会话在 LangGraph 里的 checkpoint（上下文 + 结构化记忆）。

    用 thread_id == session_id 定位；依赖不可用时静默跳过，不影响接口返回。
    """
    try:
        from gustobot.application.agents.lg_builder import _pg_pool, _pg_saver

        if _pg_saver is None or _pg_pool is None:
            return False
        await _pg_pool.open()          # 幂等：已打开则什么都不做
        await _pg_saver.adelete_thread(session_id)
        return True
    except Exception as exc:  # noqa: BLE001
        logger.warning("删除 checkpoint 失败（{}）: {}", session_id, exc)
        return False
...
async def delete_session(
    *,
    ...
    """彻底删除一个会话：消息 + 会话记录 + LangGraph checkpoint。

    以前这里是软删（只把 is_active 置为 False），数据会一直留在库里；
    现在改为硬删，侧栏的 × 就是真正移除。
    """
    session = chat_session.get(db, id=session_id)
    ...
    removed_messages = chat_message.delete_by_session(db, session_id=session_id)
    db.delete(session)
    db.commit()

    dropped = await _drop_checkpoint(session_id)
    logger.info(
        "会话已彻底删除: {}（消息 {} 条, checkpoint {}）",
        session_id, removed_messages, "已清" if dropped else "跳过",
    )
